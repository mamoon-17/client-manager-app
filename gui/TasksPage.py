...
class TasksPage(ctk.CTkFrame):
    ...
    def init_filters(self):
        ...
        ctk.CTkOptionMenu(filter_frame, variable=self.filter_priority, values=["All", "Low", "Medium", "High"],
                          command=lambda _: self.apply_filters(), fg_color=self.__WIDGET_COLOR).grid(row=0, column=0, padx=10)

        ctk.CTkCheckBox(filter_frame, text="Show Starred", variable=self.filter_starred,
                        command=self.apply_filters).grid(row=0, column=1)

        ctk.CTkCheckBox(filter_frame, text="Show Completed", variable=self.filter_completed,
                        command=self.apply_filters).grid(row=0, column=2)
    ...
    def open_add_task_dialog(self):
        ...
        try:
            ...
            self.__db.commit()
            self.__queries.log_activity("Add Task", f"Added task: {title}")
    ...
    def delete_task(self, task_id):
        try:
            self.__cursor.execute("SELECT title FROM tasks WHERE task_id = %s", (task_id,))
            title = self.__cursor.fetchone()[0]
            self.__cursor.execute("DELETE FROM tasks WHERE task_id = %s", (task_id,))
            self.__db.commit()
            self.__queries.log_activity("Delete Task", f"Deleted task: {title}")
            self.refresh_log_page()
            self.refresh_dashboard()
            self.apply_filters()
        except Exception as e:
    ...
    def toggle_star(self, task_id, new_state):
        try:
            ...
            self.__db.commit()
            action = "Star Task" if new_state else "Unstar Task"
            self.__queries.log_activity(action, f"{action} → {title}")
            self.refresh_log_page()
            self.refresh_dashboard()
            self.apply_filters()
        except Exception as e:
    ...
    def toggle_complete(self, task_id, new_state):
        try:
            ...
            self.__db.commit()
            action = "Mark Completed" if new_state else "Mark Incomplete"
            self.__queries.log_activity(action, f"{action} → {title}")
            self.refresh_log_page()
            self.refresh_dashboard()
            self.apply_filters()
    ...
    def refresh_log_page(self):
        if self.controller:
            try:
                log_page = self.controller.get_page("activityLog")
                if log_page:
                    log_page.refresh_logs()
            except Exception as e:
                print(f"[WARN] Couldn't refresh activity log page: {e}")

    def refresh_dashboard(self):
        if self.controller:
            try:
                dashboard = self.controller.get_page("dashboard")
                if dashboard:
                    dashboard.refresh_dashboard()
            except Exception as e:
                print(f"[WARN] Couldn't refresh dashboard: {e}")
